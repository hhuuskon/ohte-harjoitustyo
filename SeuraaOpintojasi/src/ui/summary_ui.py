...
class SummaryUi:
    ...
    def _summary_click(self):
        ...
        if len(result) == 0:
            self.error_message(
                "Sinulla ei ole vielä merkintöjä. Palaa edelliselle sivulle lisätäksesi niitä.")

        row_variable = 11
        for rivi in result:
            label = ttk.Label(
                master=self._frame, text=f"Kurssitunniste: {rivi[0]} | Käytetty aika: {rivi[1]} Tuntia")
    ...
    def _summary_all_click(self):
        ...
        for rivi in result:
            label = ttk.Label(
                master=self._frame, text=f"Kurssitunniste: {rivi[1]} | Päivämäärä: {rivi[3]} | Käytetty aika: {rivi[2]} Tuntia")
            label.grid(row=row_variable, column=0)
            row_variable += 1
